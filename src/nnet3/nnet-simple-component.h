...
class ScaleBackPropagationComponent: public Component {
 public:
  void Init(int32 dim, BaseFloat scale);
  explicit ScaleBackPropagationComponent(int32 dim, BaseFloat scale) {
    Init(dim, scale);
  }
  virtual int32 Properties() const {
    return kSimpleComponent|kLinearInInput;
  }
  ScaleBackPropagationComponent(): dim_(0), scale_(1.0) { }
  virtual std::string Type() const { return "ScaleBackPropagationComponent"; }
  virtual void InitFromConfig(ConfigLine *cfl);
